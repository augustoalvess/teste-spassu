...
<x-common.panels.page title="{{__('strings.professional_activity')}}">
    <form class="form-horizontal" action="{{$route}}" method="post" enctype="multipart/form-data">
        @csrf

        <div class="row">
            <div class="col-md-6 form-group">
                <x-common.fields.text-field id="description" label="{{__('strings.professional_activity')}}" value="{{old('description') ?? $data->description ?? ''}}" required="true"></x-common.fields.text-field>
            </div>
        ...
        <x-common.buttons.button title="{{__('strings.back')}}" label="{{__('strings.back')}}" class="btn btn-secondary waves-effect waves-light" icon="ri-arrow-left-circle-line loading" type="button" href="{{route('register.professional-activities')}}"></x-common.buttons.button>
    </form>
