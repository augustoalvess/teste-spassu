...
@endif
<select id="{{$id}}" name="{{$name ?? $id}}[]" class="form-control select2 select2-multiple {{$class}}" {{ $required ? 'required' : '' }} {{ $disabled ? 'readonly' : '' }} multiple="multiple" data-placeholder="{{__('strings.select')}}">
    @if (count($options) > 0)
