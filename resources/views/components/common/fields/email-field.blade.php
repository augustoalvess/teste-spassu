...
@endif
<div class="input">
    <input type="email" id="{{$id}}" name="{{$name ?? $id}}" value="{{$value}}" minlength="{{$minlength}}" maxlength="{{$maxlength}}" placeholder="{{$placeholder}}" class="form-control {{$class}}" {{ $required ? 'required' : '' }} {{ $disabled ? 'readonly' : '' }}>
    @if ($icon) 
